...
$india_template = file_get_contents('hydrocephalus.php');
...
foreach ($states as $state) {
    if ($state == "Chandigarh" || $state == "Hyderabad")
        continue; // keeping consistency
    $state_slug = strtolower(str_replace(' ', '-', $state));
    $india_grid .= '
            <a href="hydrocephalus-treatment-in-' . $state_slug . '.php" style="color: #94a3b8; text-decoration: none; font-size: 14px; display: flex; align-items: flex-start; gap: 8px; transition: color 0.2s;" onmouseover="this.style.color=\'#60a5fa\'" onmouseout="this.style.color=\'#94a3b8\'">
                ...
                Hydrocephalus Treatment in ' . $state . '
            </a>';
}
...
// Update India page (hydrocephalus.php)
if (strpos($india_template, 'Serving Patients Across India') === false) {
    // There\'s a single-line-form.php include right before the footer. We can append it before single-line-form.php
    $new_india_content = str_replace("<?php include 'single-line-form.php';?>", $india_grid . "\n<?php include 'single-line-form.php';?>", $india_template);
    file_put_contents('hydrocephalus.php', $new_india_content);
    $india_template = $new_india_content;
    echo "Updated hydrocephalus.php with state grid\n";
}
...
foreach ($state_districts as $state => $districts) {
    if ($state == "Hyderabad" || $state == "Chandigarh")
        continue;
    $state_slug = strtolower(str_replace(' ', '-', $state));
    $state_file = 'hydrocephalus-treatment-in-' . $state_slug . '.php';
    ...
    $state_content = str_replace(
        ['in India', 'in india', 'In India', 'hydrocephalus.php'],
        ["in $state", "in $state_slug", "In $state", "hydrocephalus-treatment-in-$state_slug.php"],
        $state_content_base
    );
    // URL without .php
    $state_content = str_replace('spineandbrainindia.com/hydrocephalus"', 'spineandbrainindia.com/hydrocephalus-treatment-in-' . $state_slug . '"', $state_content);
    ...
    foreach ($districts as $district) {
        ...
        $district_grid_html .= '
            <a href="hydrocephalus-treatment-in-' . $district_slug . '.php" style="color: #94a3b8; text-decoration: none; font-size: 14px; display: flex; align-items: flex-start; gap: 8px; transition: color 0.2s;" onmouseover="this.style.color=\'#60a5fa\'" onmouseout="this.style.color=\'#94a3b8\'">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#2563eb" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink: 0; margin-top: 2px;"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"></path><circle cx="12" cy="10" r="3"></circle></svg>
                Hydrocephalus Treatment in ' . $district . '
            </a>';
    }
    ...
    // Append district grid before single-line-form.php
    $state_content = str_replace("<?php include 'single-line-form.php';?>", $district_grid_html . "\n<?php include 'single-line-form.php';?>", $state_content);
    file_put_contents($state_file, $state_content);

    // Generate District Pages
    foreach ($districts as $district) {
        $district_slug = strtolower(str_replace(' ', '-', $district));
        $district_file = 'hydrocephalus-treatment-in-' . $district_slug . '.php';

        $district_content_base = $state_content_base;
        $district_content_base = str_replace(
            ['in India', 'in india', 'In India', '-in-India', 'hydrocephalus.php'],
            ["in $district", "in $district_slug", "In $district", "-in-$district_slug", "hydrocephalus-treatment-in-$district_slug.php"],
            $district_content_base
        );
        $district_content_base = str_replace('-in-india', '-in-' . $district_slug, $district_content_base);
        $district_content_base = str_replace('spineandbrainindia.com/hydrocephalus"', 'spineandbrainindia.com/hydrocephalus-treatment-in-' . $district_slug . '"', $district_content_base);

        // Append the exact same district grid
        $district_content_base = str_replace("<?php include 'single-line-form.php';?>", $district_grid_html . "\n<?php include 'single-line-form.php';?>", $district_content_base);

        file_put_contents($district_file, $district_content_base);
    }
}
echo "State and District pages generated successfully.\n";
?>