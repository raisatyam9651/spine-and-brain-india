...
$india_template = file_get_contents('nerve-graft-surgery-in-india.php');
...
foreach ($states as $state) {
    ...
    $india_grid .= '
            <a href="nerve-graft-surgery-in-' . $state_slug . '.php" style="color: #94a3b8; text-decoration: none; font-size: 14px; display: flex; align-items: flex-start; gap: 8px; transition: color 0.2s;" onmouseover="this.style.color=\'#60a5fa\'" onmouseout="this.style.color=\'#94a3b8\'">
...
if (strpos($india_template, 'Serving Patients Across India') === false) {
    ...
    file_put_contents('nerve-graft-surgery-in-india.php', $new_india_content);
    $india_template = $new_india_content;
    echo "Updated nerve-graft-surgery-in-india.php with state grid\n";
} else {
    // Save typo fixes
    file_put_contents('nerve-graft-surgery-in-india.php', $india_template);
}
...
foreach ($state_districts as $state => $districts) {
    if ($state == "Hyderabad" || $state == "Chandigarh")
        continue;
    $state_slug = strtolower(str_replace(' ', '-', $state));
    $state_file = 'nerve-graft-surgery-in-' . $state_slug . '.php';
    ...
    foreach ($districts as $district) {
        ...
        $district_grid_html .= '
            <a href="nerve-graft-surgery-in-' . $district_slug . '.php" style="color: #94a3b8; text-decoration: none; font-size: 14px; display: flex; align-items: flex-start; gap: 8px; transition: color 0.2s;" onmouseover="this.style.color=\'#60a5fa\'" onmouseout="this.style.color=\'#94a3b8\'">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#2563eb" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink: 0; margin-top: 2px;"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"></path><circle cx="12" cy="10" r="3"></circle></svg>
                Nerve Graft Surgery in ' . $district . '
            </a>';
    }
    ...
    $state_content = str_replace(
        ['in India', 'in india', 'In India', '-in-India', 'nerve-graft-surgery-in-india.php', 'nerve-graft-surgery-in-india"'],
        ["in $state", "in $state_slug", "In $state", "-in-$state_slug", "nerve-graft-surgery-in-$state_slug.php", "nerve-graft-surgery-in-$state_slug\""],
        $state_content_base
    );
    ...
    foreach ($districts as $district) {
        $district_slug = strtolower(str_replace(' ', '-', $district));
        $district_file = 'nerve-graft-surgery-in-' . $district_slug . '.php';

        $district_content = str_replace(
            ['in India', 'in india', 'In India', '-in-India', 'nerve-graft-surgery-in-india.php', 'nerve-graft-surgery-in-india"'],
            ["in $district", "in $district_slug", "In $district", "-in-$district_slug", "nerve-graft-surgery-in-$district_slug.php", "nerve-graft-surgery-in-$district_slug\""],
            $state_content_base
        );
