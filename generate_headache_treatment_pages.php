...
$india_template = file_get_contents('headache-treatment-in-India.php');
...
foreach ($states as $state) {
    ...
    $india_grid .= '
            <a href="headache-treatment-in-' . $state_slug . '.php" style="color: #94a3b8; text-decoration: none; font-size: 14px; display: flex; align-items: flex-start; gap: 8px; transition: color 0.2s;" onmouseover="this.style.color=\'#60a5fa\'" onmouseout="this.style.color=\'#94a3b8\'">
...
if (strpos($india_template, 'Serving Patients Across India') === false) {
    ...
    file_put_contents('headache-treatment-in-India.php', $new_india_content);
    $india_template = $new_india_content;
    echo "Updated headache-treatment-in-India.php with state grid\n";
}
...
foreach ($state_districts as $state => $districts) {
    ...
    $state_slug = strtolower(str_replace(' ', '-', $state));
    $state_file = 'headache-treatment-in-' . $state_slug . '.php';
    ...
    $state_content = str_replace('headache-treatment-in-india.php', 'headache-treatment-in-' . $state_slug . '.php', $state_content);
    $state_content = str_replace('headache-treatment-in-India', 'headache-treatment-in-' . $state_slug, $state_content);
    ...
    foreach ($districts as $district) {
        ...
        $district_grid_html .= '
            <a href="headache-treatment-in-' . $district_slug . '.php" style="color: #94a3b8; text-decoration: none; font-size: 14px; display: flex; align-items: flex-start; gap: 8px; transition: color 0.2s;" onmouseover="this.style.color=\'#60a5fa\'" onmouseout="this.style.color=\'#94a3b8\'">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#2563eb" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink: 0; margin-top: 2px;"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"></path><circle cx="12" cy="10" r="3"></circle></svg>
                Headache Treatment in ' . $district . '
            </a>';
    }
    ...
    foreach ($districts as $district) {
        $district_slug = strtolower(str_replace(' ', '-', $district));
        $district_file = 'headache-treatment-in-' . $district_slug . '.php';
        ...
        $district_content_base = str_replace('headache-treatment-in-india.php', 'headache-treatment-in-' . $district_slug . '.php', $district_content_base);
        $district_content_base = str_replace('headache-treatment-in-India', 'headache-treatment-in-' . $district_slug, $district_content_base);
